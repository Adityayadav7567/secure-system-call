...
def logs_data():
    ...
    query = SystemCallLog.query.filter(SystemCallLog.executed_at >= start_date)
    if not current_user.is_admin:
        query = query.filter(SystemCallLog.user_id == current_user.id)

    logs = query.all()

    # Prepare labels for each day
    labels = [(start_date + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(days)]
    statuses = ['Success', 'Error', 'Denied', 'Timeout']
    data_by_status = {status: {label: 0 for label in labels} for status in statuses}

    for log in logs:
        date_str = log.executed_at.strftime('%Y-%m-%d')
        if date_str in labels:
            status = log.status if log.status in statuses else 'Timeout'
            data_by_status[status][date_str] += 1
    ...
    datasets = [
        {
            'label': status,
            'data': [data_by_status[status][label] for label in labels],
